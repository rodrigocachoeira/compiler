...
/**
 * Set all rules about integer
 * validation
 *
 * @author Rodrigo Cachoeira
 * @version 1.0.0
 */
public class IntegerLexical implements RuleInterface{
    ...
    /**
     * Define if number is a float pattern
     */
    ...
    public RuleInterface goTo() throws InvalidEndPointException, InvalidIntegerException, InvalidFloatException {
        ...
        if (EndLexical.regexValidation(nextValue)) { //end
            this.validated = true;
            return this; //end of recursion.goTo();.goTo();
        }
     ...
     */
    @Override
    public IntegerLexical run() throws InvalidEndPointException, InvalidIntegerException, InvalidFloatException {
        this.goTo();

        if (IntegerLexical.isFloat) {
            AnalyzerMessage.setMessage("Float found");
        } else {
